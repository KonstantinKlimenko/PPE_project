# Детекция СИЗ на строительных площадках

Небольшой pet-проект по компьютерному зрению для портфолио.  
Модель ищет на изображениях строительные каски, сигнальные жилеты и головы.
...
На вход подается фотография со стройки или промышленной площадки.  
...
Используется YOLO11n - легкая модель для детекции объектов.  
Я выбрал ее, потому что она быстро обучается, подходит для локального запуска и хорошо работает для демо-проекта.
